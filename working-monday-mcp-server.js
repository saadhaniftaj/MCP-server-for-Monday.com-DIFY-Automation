...
console.log('🔓 Working Monday.com MCP Server - NO AUTHENTICATION REQUIRED');

// Health check
app.get('/health', (req, res) => {
  res.json({
    status: 'ok',
    message: 'Working Monday.com MCP Server is running',
    ...
    return res.json({
      ...
      result: {
        content: [
          {
            type: 'text',
            text: result ? JSON.stringify(result, null, 2) : 'No data returned'
          }
        ]
      }
    });
...
app.listen(PORT, () => {
  console.log(`🚀 Working Monday.com MCP Server running on port ${PORT}`);
