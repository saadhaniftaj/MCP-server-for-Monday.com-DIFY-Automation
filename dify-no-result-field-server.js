...
console.log('🔓 Dify-No-Result-Field Server - NO AUTHENTICATION REQUIRED');

// Health check
app.get('/health', (req, res) => {
  res.json({
    status: 'ok',
    message: 'Dify-No-Result-Field Server is running',
...
app.post('/', (req, res) => {
  ...
  // Handle notifications/initialized - send response without result field
  if (method === 'notifications/initialized') {
    console.log('📤 Handling notifications/initialized - sending response without result field');
    const response = {
      jsonrpc: '2.0',
      id: id || 0
    };
...
app.listen(PORT, () => {
  console.log(`🚀 Dify-No-Result-Field Server running on port ${PORT}`);
