...
app.get('/health', (req, res) => {
  res.json({
    status: "ok",
    message: "Dify-Railway Server is running",
    auth: "NO AUTHENTICATION REQUIRED"
  });
});

// Main MCP endpoint - Railway optimized
app.post('/', (req, res) => {
  ...
  // Handle different methods - Railway optimized
  switch (method) {
    case 'initialize':
      return res.json({
        jsonrpc: "2.0",
    ...
    case 'notifications/initialized':
      // Quick response for notifications to prevent timeouts
      return res.json({
        jsonrpc: "2.0",
        id: 0,
        result: {
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 Dify-Railway Server running on port ${PORT}`);
  console.log(`⚡ Railway optimized - quick responses to prevent timeouts`);
