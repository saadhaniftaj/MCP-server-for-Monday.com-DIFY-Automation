...
app.get('/health', (req, res) => {
  res.json({
    ...
    note: "Workaround for Dify.ai validation bug"
  });
});

// Main MCP endpoint - Dify.ai compatible responses
app.post('/', (req, res) => {
  ...
  // Handle different methods - Dify.ai compatible responses
  switch (method) {
    case 'initialize':
      return res.json({
        jsonrpc: "2.0",
    ...
    case 'notifications/initialized':
      // 🚨 DIFY.AI WORKAROUND: Use id: 0 instead of null to satisfy their buggy validation
      return res.json({
        jsonrpc: "2.0",
        id: 0, // WORKAROUND: Dify.ai rejects null but accepts 0
        result: {
          status: "ok",
          protocol_version: "2025-03-26",
...
app.use((err, req, res, next) => {
  ...
  res.status(500).json({
    jsonrpc: "2.0",
    id: 0, // Use 0 instead of null for Dify.ai compatibility
    error: {
      code: -32603,
...
app.listen(PORT, () => {
  ...
  console.log(`⚡ WORKAROUND: id: 0 for notifications (Dify.ai bug)`);
  console.log(`🔧 Protocol version: 2025-03-26`);
  console.log(`🔗 Health check: http://localhost:${PORT}/health`);
  console.log(`🐛 Note: This works around Dify.ai's validation bug`);
}); 