...
// Middleware - optimized for HTTP 204 responses
app.use(cors());
...
app.get('/health', (req, res) => {
  res.json({
    status: "ok",
    message: "Dify-204 Server is running",
    auth: "NO AUTHENTICATION REQUIRED"
  });
});

// Main MCP endpoint - HTTP 204 for notifications
app.post('/', (req, res) => {
  ...
  // Handle different methods - HTTP 204 for notifications
  switch (method) {
    case 'initialize':
      return res.json({
        jsonrpc: "2.0",
    ...
    case 'notifications/initialized':
      // HTTP 204 No Content - proper MCP specification for notifications
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 Dify-204 Server running on port ${PORT}`);
  console.log(`⚡ HTTP 204 for notifications - proper MCP specification`);
