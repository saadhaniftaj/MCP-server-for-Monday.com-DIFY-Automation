...
console.log('🔓 Dify-MCP-Spec-Compliant Server - NO AUTHENTICATION REQUIRED');

// Health check
app.get('/health', (req, res) => {
  res.json({
    status: 'ok',
    message: 'Dify-MCP-Spec-Compliant Server is running',
...
app.post('/', (req, res) => {
  ...
  // Handle notifications/initialized - according to MCP spec, this should be a notification (no response)
  if (method === 'notifications/initialized') {
    console.log('📤 Handling notifications/initialized - sending HTTP 204 No Content as per MCP spec');
...
app.listen(PORT, () => {
  console.log(`🚀 Dify-MCP-Spec-Compliant Server running on port ${PORT}`);
