...
console.log('🔓 Dify-Avoid-Cleanup Server - NO AUTHENTICATION REQUIRED');

// Health check
app.get('/health', (req, res) => {
  res.json({
    status: 'ok',
    message: 'Dify-Avoid-Cleanup Server is running',
...
app.post('/', (req, res) => {
  ...
  // Handle notifications/initialized - send a response that might avoid cleanup
  if (method === 'notifications/initialized') {
    console.log('📤 Handling notifications/initialized - sending response that might avoid cleanup');
    const response = {
      jsonrpc: '2.0',
      id: id || 0,
      result: {
...
app.listen(PORT, () => {
  console.log(`🚀 Dify-Avoid-Cleanup Server running on port ${PORT}`);
