...
// Middleware - optimized for no response notifications
app.use(cors());
...
app.get('/health', (req, res) => {
  res.json({
    status: "ok",
    message: "Dify-No-Response Server is running",
    auth: "NO AUTHENTICATION REQUIRED"
  });
});

// Main MCP endpoint - No response for notifications
app.post('/', (req, res) => {
  ...
  // Handle different methods - No response for notifications
  switch (method) {
    case 'initialize':
      return res.json({
        jsonrpc: "2.0",
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 Dify-No-Response Server running on port ${PORT}`);
  console.log(`⚡ NO RESPONSE for notifications - prevents ALL cleanup errors`);
