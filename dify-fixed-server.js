...
// Middleware - optimized for proper JSON-RPC responses
app.use(cors());
...
app.get('/health', (req, res) => {
  res.json({
    status: "ok",
    message: "Dify-Fixed Server is running",
    version: "1.0.0",
    protocol_version: "2025-03-26"
  });
});

// Main MCP endpoint - FIXED JSON-RPC responses
app.post('/', (req, res) => {
  ...
  // Handle different methods - FIXED JSON-RPC responses
  switch (method) {
    case 'initialize':
      return res.json({
        jsonrpc: "2.0",
        id: id,
        result: {
          protocolVersion: "2025-03-26", // Match Dify's expected version
          capabilities: {
            tools: {},
            sampling: {},
            roots: {
    ...
    case 'notifications/initialized':
      // 🚨 CRITICAL FIX: id MUST be null for notifications (not 0)
      return res.json({
        jsonrpc: "2.0",
        id: null, // FIXED: null instead of 0
        result: {
          status: "ok",
          protocol_version: "2025-03-26" // Match Dify's expected version
        }
      });
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 Dify-Fixed Server running on port ${PORT}`);
