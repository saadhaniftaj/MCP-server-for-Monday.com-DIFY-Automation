...
app.get('/health', (req, res) => {
  res.json({
    status: "ok",
    message: "Dify-Minimal Server is running",
    auth: "NO AUTHENTICATION REQUIRED"
  });
});

// Main MCP endpoint - Minimal responses
app.post('/', (req, res) => {
  ...
  // Handle different methods - Minimal responses
  switch (method) {
    case 'initialize':
      return res.json({
        jsonrpc: "2.0",
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 Dify-Minimal Server running on port ${PORT}`);
  console.log(`⚡ Minimal responses - prevents timeouts and cleanup errors`);
