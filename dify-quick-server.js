...
// Middleware - optimized for quick responses
app.use(cors());
...
app.get('/health', (req, res) => {
  res.json({
    status: "ok",
    message: "Dify-Quick Server is running",
    auth: "NO AUTHENTICATION REQUIRED"
  });
});

// Main MCP endpoint - Quick responses for all requests
app.post('/', (req, res) => {
  ...
  // Handle different methods - Quick responses for all
  switch (method) {
    case 'initialize':
      return res.json({
        jsonrpc: "2.0",
    ...
    case 'notifications/initialized':
      // Quick response for notifications to prevent 504 timeouts
      return res.json({
        jsonrpc: "2.0",
        id: 0,
        result: {
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 Dify-Quick Server running on port ${PORT}`);
  console.log(`⚡ Quick responses for all requests - prevents 504 timeouts`);
