...
console.log('🔓 Dify-Minimal-JSON Server - NO AUTHENTICATION REQUIRED');

// Health check
app.get('/health', (req, res) => {
  res.json({
    status: 'ok',
    message: 'Dify-Minimal-JSON Server is running',
...
app.post('/', (req, res) => {
  ...
  if (method === 'notifications/initialized') {
    console.log('📤 Handling notifications/initialized - sending minimal JSON');
    const response = {
      jsonrpc: '2.0',
      id: id || 0,
      result: {}
...
app.listen(PORT, () => {
  console.log(`🚀 Dify-Minimal-JSON Server running on port ${PORT}`);
