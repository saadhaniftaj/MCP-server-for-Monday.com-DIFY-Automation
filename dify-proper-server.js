...
app.get('/health', (req, res) => {
  res.json({
    status: "ok",
    message: "Dify-Proper Server is running",
    auth: "NO AUTHENTICATION REQUIRED"
  });
});

// Main MCP endpoint - Proper JSON-RPC responses
app.post('/', (req, res) => {
  ...
  // Handle different methods - Proper JSON-RPC responses
  switch (method) {
    case 'initialize':
      return res.json({
        jsonrpc: "2.0",
    ...
    case 'notifications/initialized':
      // Proper JSON-RPC response with all required fields
      return res.json({
        jsonrpc: "2.0",
        id: 0,
        result: {
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 Dify-Proper Server running on port ${PORT}`);
  console.log(`⚡ Proper JSON-RPC responses for all methods`);
