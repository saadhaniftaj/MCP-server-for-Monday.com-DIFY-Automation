...
// Middleware - optimized for empty responses
app.use(cors());
...
app.get('/health', (req, res) => {
  res.json({
    status: "ok",
    message: "Dify-Empty Server is running",
    auth: "NO AUTHENTICATION REQUIRED"
  });
});

// Main MCP endpoint - Empty responses
app.post('/', (req, res) => {
  ...
  // Handle different methods - Empty responses
  switch (method) {
    case 'initialize':
      return res.json({
        jsonrpc: "2.0",
    ...
    case 'notifications/initialized':
      // Empty JSON object - minimal response
      return res.json({});
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 Dify-Empty Server running on port ${PORT}`);
  console.log(`⚡ Empty JSON response for notifications`);
