...
app.get('/health', (req, res) => {
  ...
  res.json({
    status: "ok",
    message: "Dify-Bypass Server is running",
    version: "1.0.0",
    protocol_version: "2025-03-26",
    note: "Bypasses Dify.ai notification issues"
  });
});

// Main MCP endpoint - Bypass Dify.ai issues
app.post('/', (req, res) => {
  ...
  // Handle different methods - Bypass Dify.ai issues
  switch (method) {
    case 'initialize':
      return res.json({
        jsonrpc: "2.0",
    ...
    case 'notifications/initialized':
      // 🚨 FIX 1: Bypass Dify's Notification System - Return IMMEDIATE 200 OK with EMPTY BODY
      res.setHeader('Content-Type', 'text/plain');
      return res.status(200).send(''); // Empty response prevents all cleanup errors
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 Dify-Bypass Server running on port ${PORT}`);
  console.log(`⚡ FIX 1: Bypass notifications with empty response`);
  console.log(`🔧 Protocol version: 2025-03-26`);
  console.log(`🔗 Health check: http://localhost:${PORT}/health`);
  console.log(`🌐 CORS enabled for Dify.ai domains`);
  console.log(`📝 Note: This bypasses Dify.ai's notification issues`);
}); 