...
console.log('🔓 Dify-Ignore-Cleanup Server - NO AUTHENTICATION REQUIRED');

// Health check
app.get('/health', (req, res) => {
  res.json({
    status: 'ok',
    message: 'Dify-Ignore-Cleanup Server is running',
...
app.post('/', (req, res) => {
  ...
  // Handle notifications/initialized - send a response that might be ignored during cleanup
  if (method === 'notifications/initialized') {
    console.log('📤 Handling notifications/initialized - sending response that might be ignored during cleanup');
    const response = {
      jsonrpc: '2.0',
      id: id || 0,
      result: {
...
app.listen(PORT, () => {
  console.log(`🚀 Dify-Ignore-Cleanup Server running on port ${PORT}`);
