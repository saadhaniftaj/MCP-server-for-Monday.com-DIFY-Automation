...
app.post('/api/monday/update-email', async (req, res) => {
  ...
  try {
    // Real Monday.com GraphQL mutation with correct JSON format
    const mutation = `
      mutation {
        change_column_value(
          board_id: ${boardId},
...
// Real Monday.com column update with correct format
...
app.listen(PORT, () => {
  console.log(`🚀 Real Monday.com Server (Fixed) running on port ${PORT}`);
